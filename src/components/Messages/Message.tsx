...
const isOwnMessage = (message: IMessage , user: IUser | null) => {
  ...
  console.log(`Şuan gönderen ${message.sender.email}`)
  console.log(`Şuan kullanıcı ${user?.email}`)
      ...
      <Comment>
           <Comment.Avatar src={message.sender?.avatar ?? 'http://www.gravatar.com/avatar/?=identicon'} />
            <Comment.Content className={isOwnMessage(message,currentUser)}>
                ...
                {isImage(message) ? (<Image src={message.content} className="message__image"/>) :(  <Comment.Text>{message.content}</Comment.Text>) }
